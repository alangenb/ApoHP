...
if isempty(fsz), fsz=10; end % default is probably 10
if length(fsz)>1, fsz=max(fsz); end
if fsz<6
  xthresh = 0.1;
...
blank = (strcmp('',txt));
if any(blank)   % replace blank text with a single space (was causing errors due to zero size ("extent") of the labels
  txt(blank) = repmat({' '},sum(blank),1);
end
...
for t=1:maxtries
  ...
  for i=1:length(o1), i1=o1(i); i2=o2(i);
    ...
    if any(isnan(pos1))||any(isnan(pos2)), error('problem with textfit: zero label size?'); end
