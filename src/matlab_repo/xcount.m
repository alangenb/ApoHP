function [ct,ua,ub] = xcount(a, b, sort_by_frequency, truncate_length, pct_sign_prefix)
...
if exist('truncate_length','var') && ~isempty(truncate_length)
  for i=1:length(ua)
...
try
  ct = hist2d_fast_wrapper(uaj,ubj,1,nua,1,nub);
catch
  ct = zeros(nua,nub);
  for aj=1:nua
    for bj=1:nub
...
if exist('sort_by_frequency', 'var') && ~isempty(sort_by_frequency) && sort_by_frequency~=0
...
if nargout==0
  ...
  fprintf('\n');
  for aj=1:nua+2
    if aj==nua+2, if pct_sign_prefix, fprintf('%%'); end; fprintf('\n'); end
    if pct_sign_prefix, fprintf('%%'); end
    fprintf('    ');
    for bj=1:nub+2
      if bj==2, fprintf('  '); end
