...
%MAXCOLS=50;  % until 2019-12-01
MAXCOLS=60;  % after 2019-12-01
multi_flag = false(ncol,1);
for c=1:ncol
%  if size(x{c},1)>1 && size(x{c},2)>1 && size(x{c},2)<=MAXCOLS && ndims(x{c})<3 && (isnumeric(x{c}) || islogical(x{c}))
% 2018-07-24  FIXED BUG that was printing only the first column of multi-column fields, in cases of printing only a single row
  if size(x{c},2)>1 && size(x{c},2)<=MAXCOLS && ndims(x{c})<3 && (isnumeric(x{c}) || islogical(x{c}))
    multi_flag(c) = true;
  ...
  for c=1:ncol
    if i==0
      ...
      if size(x{c},1)>1 && size(x{c},2)>1
        ...
        if size(x{c},2)<=MAXCOLS && ((isnumeric(x{c}) || islogical(x{c})) && ndims(x{c})<3)
        ...
        elseif iscell(x{c})
          val = x{c}{i};
          if ischar(val) && size(val,1)==1  % strings
            MAXSTRINGLENGTH=max(20,ceil(PAGEWIDTH/(ncol/10)));  % introduced 2019-12-17
            if length(val)<=MAXSTRINGLENGTH
...
for pi=1:npages
  ...
  for j=1:length(idx)
    ...
    for c=cols
      ...
      if c<ncol, subfwritepad('',nsp); end
    end
    subfprintf('\n');
  end
